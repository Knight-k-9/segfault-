...
- 只有 actual_x > 2.5m 时才先发布零速度，再发送 /qr_success 完成交接；
...
class Task1NavAvoidNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            ...
            f'{self.channel_handoff_min_x:.2f}m时停车并发布 /qr_success'
        )
        self.get_logger().info(
    ...
    def qr_result_cb(self, msg):
        ...
        self.get_logger().warn(
            f'📥 已提前缓存二维码结果：“{result}”；{position_text}。'
            '任务1继续巡航，越过交接门槛后再停车并发布 /qr_success'
        )
