...
class FileTrackFollowerNode(Node):
    def __init__(self):
        ...
        direction_str = 'cw' if self.is_clockwise else 'ccw'
        self.dense_path = self.load_and_stitch_track(self.track_file, direction=direction_str)
        ...
        if not self.dense_path:
            self.get_logger().error("❌ 轨迹文件读取失败或为空，程序即将退出！")
            return
        ...
        self.get_logger().info(f"🚀 [IMU预测补偿阿克曼控制] 启动 | 轴距: {self.wheelbase}m | 前移量: {self.imu_to_front}m")
    ...
    def control_loop(self):
        if self.is_finished or not self.dense_path:
            return
