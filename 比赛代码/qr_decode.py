...
输出：
  /qr_direction_result         std_msgs/msg/String
      仅发布“顺时针”或“逆时针”，保持现有导航链路兼容。
  /qr_full_result              std_msgs/msg/String
...
class QRDecodeNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('full_result_topic', '/qr_full_result')
        ...
        # TRANSIENT_LOCAL 会缓存最后一次结果；即使 OLED 节点稍晚启动，
        # 仍然可以立即收到二维码数字。
        ...
        self.get_logger().info(
            f'方向结果话题：{self.result_topic} (订阅+发布)'
        )
        self.get_logger().info(
            f'完整结果话题：{self.full_result_topic} (发布)'
        )
        self.get_logger().info(
        ...
        for method_name, candidate in self._build_decode_variants(roi):
            ...
            for obj in objects:
                ...
                if content.isdigit():
                    ...
                    return decoded_any, numeric_number, numeric_direction
    ...
    def yolo_cb(self, msg):
        ...
        for index, det in enumerate(detections):
            ...
            for ratio in self.expand_ratios:
                ...
                decoded_any, number, direction = self._decode_one_roi(
                    roi,
                    ratio,
                )
                ...
                if number is not None and direction is not None:
                    ...
                    # 原方向话题保持不变，避免影响 task_xian.py 等导航节点。
                    result = String()
                    result.data = direction
                    self.result_pub.publish(result)

                    # 新增完整结果话题，保留二维码原始数字供展示节点使用。
                    full_result = String()
                    full_result.data = json.dumps(
                        {
                            'number': number,
                            'direction': direction,
                        },
                    ...
                    self.full_result_pub.publish(full_result)

                    self.decode_success_count += 1
                    self.get_logger().info(
                        f'📤 已发布 {self.result_topic}: {direction}；'
                        f'{self.full_result_topic}: {full_result.data}，'
                        '本节点停止'
                    )
                    return
